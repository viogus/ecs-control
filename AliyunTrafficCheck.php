...
class AliyunTrafficCheck
{
    ...
    public function getAllManagedInstances($sync = false)
    {
        ...
        foreach ($accounts as $account) {
            $allInstances[] = $this->responseBuilder->buildInstanceSnapshot($account, $threshold, $userInterval, false, true, $sync);
        }

        $pendingAccounts = $this->configManager->getPendingReleaseAccounts();
        foreach ($pendingAccounts as $account) {
            $snap = $this->responseBuilder->buildInstanceSnapshot($account, $threshold, $userInterval, false, true, $sync);
