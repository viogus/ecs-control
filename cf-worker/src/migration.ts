...
export async function importFromDocker(db: D1Database, encKey: string, data: MigrationExport, opts: ImportOptions = {}): Promise<void> {
  ...
  // Phase 1: stage new accounts — tag old (is_deleted=3), insert new (is_deleted=2)
  await db.prepare("UPDATE accounts SET is_deleted = 3 WHERE is_deleted = 0").run();

  try {
    const insertSql = `INSERT INTO accounts (access_key_id,access_key_secret,region_id,instance_id,max_traffic,instance_status,remark,site_type,group_key,instance_name,instance_type,internet_max_bandwidth_out,public_ip,public_ip_mode,eip_allocation_id,eip_address,eip_managed,cpu,memory,os_name,schedule_enabled,schedule_start_enabled,schedule_stop_enabled,start_time,stop_time,schedule_blocked_by_traffic,traffic_billing_month,is_deleted) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,2)`;
    ...
    await db.prepare("UPDATE accounts SET is_deleted = 0 WHERE is_deleted = 3").run();
    throw e;
  }

  // Phase 2: write settings (with full rollback on failure)
  try {
    ...
    // Rollback: restore old accounts, remove new ones
    await db.prepare("UPDATE accounts SET is_deleted = 0 WHERE is_deleted = 3").run();
    await db.prepare("DELETE FROM accounts WHERE is_deleted = 2").run();
    throw e;
  }

  // Phase 3: commit — remove old accounts, promote new ones
  await db.prepare('DELETE FROM accounts WHERE is_deleted = 3').run();
  await db.prepare('UPDATE accounts SET is_deleted = 0 WHERE is_deleted = 2').run();
}
