...
- **告警去重**: `MonitorService::notifyTrafficWarningWithCooldown` dedupes traffic alerts per account per billing month (1h cooldown); `AccountRefresher` writes failure cooldown keys (`cdt_failure_at_*`) so failed CDT queries back off 5 min; if CDT stays unavailable past the 15-min breaker-exemption window, `MonitorService::notifyCdtUnavailable` alerts once per outage (`Helpers::cdtNotifyKeySuffix` → `cdt_failure_notified_at_ak-*`, AK-scoped because CDT is queried per AK, so a multi-instance group sends one alert, not N). The success marker is written **only when delivery succeeded**; a failed push records `cdt_failure_notify_attempt_at_*` and retries after 30 min, so a broken channel cannot silence the alert permanently. `AccountRefresher` clears all of these on recovery — that window skips the breaker, so the protection gap must be announced.
- **CDT 错误分类**: `Helpers::isNetworkError()` keys off the SDK client error-code whitelist (`SDK.ServerUnreachable`, `SDK.HostNotFound` — verified against `vendor/alibabacloud/client`; probing DNS failure / connection refused / connect timeout shows all three surface as `SDK.ServerUnreachable`) plus message hints for non-SDK errors, keeping transport failures out of the credential path: they surface as `traffic_api_status=timeout`, never `auth_error`, so network blips cannot suspend auto-stop protection. `Helpers::isPermissionError()` maps RAM-denied responses (`Forbidden.*`, `NoPermission`, …) to `permission_denied`; only `auth_error` suspends protection (`AccountRefresher::isCredentialInvalidTrafficStatus`). Idempotent reads opt into one transport retry via `RetryHandler::execute($fn, 'getTraffic', 3, 1)`. Tests: `tests/HelpersTest.php`, `tests/RetryHandlerTest.php`, `tests/MonitorServiceTest.php`.
